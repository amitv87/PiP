...
httpd_add_connection(httpd_t *httpd, int fd, unsigned char *local, int local_len, unsigned char *remote, int remote_len)
{
    ...
    user_data = httpd->callbacks.conn_init(httpd->callbacks.opaque, &httpd->connections[i], local, local_len, remote, remote_len);
    if (!user_data) {
...
httpd_stop(httpd_t *httpd)
{
    ...
    httpd->running = 0;
    if(httpd->thread) THREAD_CANCEL(httpd->thread);
    MUTEX_UNLOCK(httpd->run_mutex);
