...
static void *
conn_init(void *opaque, http_connection_t* http_conn, unsigned char *local, int locallen, unsigned char *remote, int remotelen) {
    raop_t *raop = opaque;
    raop_conn_t *conn;
...
//        raop->callbacks.conn_init(raop->callbacks.cls, &conn->conn);
//    }
...
conn_request(void *ptr, http_request_t *request, http_response_t **response) {
    ...
    *response = http_response_init("RTSP/1.0", 200, "OK");
    ...
    logger_log(conn->raop->logger, LOGGER_DEBUG, "con: %p, Handling request %s with URL %s", &conn->conn, method, url);
    ...
    if (!strcmp(method, "GET") && !strcmp(url, "/info")) {
       ...
       logger_log(conn->raop->logger, LOGGER_INFO, "*** (such as Apple Configurator or a third-party MDM tool).");
        ...
        if (conn->raop->callbacks.conn_init && !conn->call_init_cb)conn->call_init_cb += 1;
        ...
        if (conn->raop->callbacks.conn_teardown) {
             conn->raop->callbacks.conn_teardown(conn->raop->callbacks.cls, &teardown_96, &teardown_110, &conn->conn);
        }
    ...
    if(conn->call_init_cb == 1){
      conn->call_init_cb += 1;
      conn->raop->callbacks.conn_init(conn->raop->callbacks.cls, &conn->conn);
    }
...
conn_destroy(void *ptr) {
    ...
    if (conn->raop->callbacks.conn_destroy && conn->conn.usr_data) {
        conn->raop->callbacks.conn_destroy(conn->raop->callbacks.cls, &conn->conn);
    }
...
void
raop_stop_conn(raop_connection_t* _conn){
  raop_conn_t *conn = _conn->priv;
  void httpd_remove_connection(httpd_t *httpd, void *connection);
  httpd_remove_connection(conn->raop->httpd, conn->http_conn);
}
